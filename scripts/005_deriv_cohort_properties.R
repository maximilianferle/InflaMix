...
df_hm <- dfc1  %>% select(c(record_id, d0_cluster, starts_with("d0_"))) %>%
  select(!record_id) %>% pivot_longer(!d0_cluster, names_to = "labs", values_to = "result_value") %>%
  group_by(d0_cluster, labs) %>%
  summarize(mean_result=mean(result_value)) %>% ungroup() %>%
...
# Actual plots
fig1f_suv <-  dxburd_plot(dfe1, "bl_suvmax", "SUVmax", "FALSE") + annotate("text", label ="FDR Adj. p < 0.001", x =1.10, y = 1.7, size=4)
fig1e_mtv <-  dxburd_plot(dfe1, "bl_mtv", "Metabolic Tumor Volume", "FALSE")+ annotate("text", label ="FDR Adj. p < 0.001", x =1.10, y = 3.8, size=4)
fig1d_ldh <-  dxburd_plot(dfe1, "preld_ldh", "Lactate Dehydrogenase", "FALSE")+ annotate("text", label ="FDR Adj. p < 0.001", x =1.10, y = 3.9, size=4)
