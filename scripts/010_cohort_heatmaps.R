...
dfb1 <- left_join(df_all, df_labs_all %>% select(!cohort), by="record_id") %>%
  ...
  filter(cohort=="Non-Hodgkin Lymphoma Validation") %>% # Change this filter to generate a heatmap for each cohort
...
dfc2 <- dfb %>% rename(d0_cluster=cluster) %>% mutate(noLCS_d0_ddimer=ifelse(center=="Sheba Medical Center",noLCS_d0_ddimer/1000, noLCS_d0_ddimer))
...
medhm_inf <- Heatmap(as.matrix(df_hm[,2]), width=unit(4.1, "cm"), col=cp1,
                     ...
                     cell_fun = function(j, i, x, y, width, height, fill) {
                       grid.text(sprintf("%s", df_words[i, 2]), x, y, gp = gpar(fontsize = 10.5))
...
medhm_ninf <- Heatmap(as.matrix(df_hm[,1]), width=unit(4.1, "cm"), col=cp1,
                      ...
                      row_order=ro,
                      cell_fun = function(j, i, x, y, width, height, fill) {
                        grid.text(sprintf("%s", df_words[i, 1]), x, y, gp = gpar(fontsize = 10.5))
                      }, show_heatmap_legend = FALSE,
                      show_row_names = FALSE,
...
corehm <- Heatmap(as.matrix(df_full_hm), col=cp1, column_gap=unit(1, "cm"),
                  ...
                  column_title="IV. Mantle Cell and Follicular Lymphoma Validation Cohort Patients", # Change this title to reflect the cohort
...
efig3a_d_fin <- plot_grid(efig3a_d, labels=c(""), scale=0.93, label_size=30)
efig3a_d_fin
